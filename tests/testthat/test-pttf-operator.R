...
test_that("PTTF symmetric tensor bases have expected dimensions", {
  expect_equal(geosmooth:::.pttf.operator.q(1L, 3L), 1L)
  expect_equal(geosmooth:::.pttf.operator.q(2L, 3L), 4L)
  expect_equal(geosmooth:::.pttf.operator.q(3L, 3L), 10L)

  b2 <- geosmooth:::.pttf.symmetric.tensor.basis(3L, 2L, "hs")
  expect_equal(b2$q, 6L)
  expect_equal(b2$multiplicity, c(1L, 2L, 2L, 1L, 2L, 1L))

  b3 <- geosmooth:::.pttf.symmetric.tensor.basis(3L, 3L, "hs")
...
test_that("PTTF tensor design contracts symmetric tensors correctly", {
  ...
  for (scaling in c("hs", "raw")) {
    b1 <- geosmooth:::.pttf.symmetric.tensor.basis(2L, 1L, scaling)
    b2 <- geosmooth:::.pttf.symmetric.tensor.basis(2L, 2L, scaling)
    b3 <- geosmooth:::.pttf.symmetric.tensor.basis(2L, 3L, scaling)

    h <- geosmooth:::.pttf.tensor.coord.from.full(H, b2)
    phi2 <- geosmooth:::.pttf.symmetric.tensor.direction.design(u, 1L, 2L, scaling)
    expect_equal(as.vector(phi2 %*% h),
                 geosmooth:::.pttf.tensor.coord.from.full(as.vector(H %*% u), b1),
                 tolerance = 1e-12,
                 info = scaling)

    tcoord <- geosmooth:::.pttf.tensor.coord.from.full(T, b3)
    phi3 <- geosmooth:::.pttf.symmetric.tensor.direction.design(u, 2L, 3L, scaling)
    contracted <- geosmooth:::.pttf.tensor.contract.last(T, u)
    expect_equal(as.vector(phi3 %*% tcoord),
                 geosmooth:::.pttf.tensor.coord.from.full(contracted, b2),
                 tolerance = 1e-12,
                 info = scaling)
  }
})
...
test_that("PTTF tensor transport matches explicit matrix formulas", {
  ...
  for (scaling in c("hs", "raw")) {
    b2 <- geosmooth:::.pttf.symmetric.tensor.basis(2L, 2L, scaling)
    h <- geosmooth:::.pttf.tensor.coord.from.full(H, b2)
    K2 <- geosmooth:::.pttf.symmetric.tensor.transport(O, 2L, scaling)
    expect_equal(as.vector(K2 %*% h),
                 geosmooth:::.pttf.tensor.coord.from.full(O %*% H %*% t(O), b2),
                 tolerance = 1e-12,
                 info = scaling)
  }
})
