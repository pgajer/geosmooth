local({
  api <- list(solve = getFromNamespace("quadform_geodesics_solver", "geosmooth"))
...
test_that("normal execution needs no Python, modifies no R RNG and creates no files", {
  original_rng <- get0(".Random.seed", envir = .GlobalEnv, inherits = FALSE)
  on.exit({
    if (is.null(original_rng)) {
      if (exists(".Random.seed", envir = .GlobalEnv, inherits = FALSE))
        rm(".Random.seed", envir = .GlobalEnv)
    } else assign(".Random.seed", original_rng, envir = .GlobalEnv)
  }, add = TRUE)
...
test_that("the installed interface stays internal and never compiles at runtime", {
  expect_false("quadform_geodesics_solver" %in% getNamespaceExports("geosmooth"))
  expect_false(grepl("sourceCpp|QGC_BUILD_CACHE|reticulate", paste(deparse(body(api$solve)),collapse = "")))
  expect_true("rcpp_quadform_geodesics_solver" %in% names(asNamespace("geosmooth")))
})
})
