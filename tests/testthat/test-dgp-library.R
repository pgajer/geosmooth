...
test_that("G6 clipping is active under a strong log-odds (clip-fidelity GATE)", {
  spec <- synthetic.registry.spec("G6", list(amplitude = 6))
  ds <- materialize.synthetic(spec, n = 600, seed = 1,
                              rng.policy = "legacy")
  expect_true(all(ds$truth >= 0.05 - 1e-12 & ds$truth <= 0.95 + 1e-12))
  ...
  spec2 <- synthetic.registry.spec(
    "G6", list(amplitude = 6, clip = c(0.1, 0.9)))
  ds2 <- materialize.synthetic(spec2, n = 600, seed = 1,
                               rng.policy = "legacy")
  expect_true(all(ds2$truth >= 0.1 - 1e-12 & ds2$truth <= 0.9 + 1e-12))
  expect_true(any(ds2$truth == 0.9))
  expect_true(any(ds2$truth == 0.1))
  expect_error(
    suppressWarnings(dgp.g6(n = 20, eta.fn = function(x) x[, 1])),
    "Arbitrary eta.fn closures")
})
...
# The former `inst/dgp_registry` table is retained only as legacy provenance.
# Canonical replay and checksum coverage lives in test-synthetic-api.R.
