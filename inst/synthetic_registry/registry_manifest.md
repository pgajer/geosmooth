...
- `geometries.csv`, `samplings.csv`, `truths.csv`, and `responses.csv` are
  normalized component ledgers. Each row contains an XDR-v3 serialized
  component payload, its stable ID, kind, version, and SHA-256. The public
  resolver reconstructs recipes from these rows rather than dispatching on
...
- `checksums.csv` states the checksum payload, XDR serialization, and
  complete environment scope for frozen instances, including the compiler,
  operating system, RNG kind, BLAS/LAPACK, math runtime, registry/evaluator
  versions, dependency versions, and scientific tolerances.
...
Tests require every foreign key to resolve exactly once, every serialized
component and recipe hash to reproduce, every fixture to validate, and every
active frozen checksum to replay. Do not edit generated normalized ledgers
without also changing their owning component or catalog source.
