...
# dev/methods/lps/runs/e1_10_acceptance/ binding the committed study outputs to the run:
...
# battery summary. Unlike generated audit-artifact harness bundles,
# this manifest is committed so the acceptance evidence is tracked.
#
# Usage: bash dev/methods/lps/ci/e1_10_acceptance_manifest.sh
# =============================================================================
set -uo pipefail
cd "$(git rev-parse --show-toplevel)" || exit 2

OUT="dev/methods/lps/runs/e1_10_acceptance"
...
SRC_FILES=(R/lps.R R/lps_cv_utils.R dev/methods/lps/ci/e1_10_nested_grouped_cv.R \
...
Rscript -e '
b <- read.csv("dev/methods/lps/runs/e1_10_acceptance/e1_10_b_grouped_cases.csv")
b <- b[, c("rho.nominal","replicate","seed.base","realized.icc")]
write.csv(b, "dev/methods/lps/runs/e1_10_acceptance/realized_rho_per_replicate.csv",
          row.names = FALSE)
' 2>&1 || true

{
  ...
  echo "[end status]"
  echo
  echo "--- source-hash verification against Part B audit (dev/methods/lps/audits/e1_10_partB_audit_2026-06-14.md) ---"
