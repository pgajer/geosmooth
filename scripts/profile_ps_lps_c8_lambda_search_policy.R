...
out.dir <- file.path(
  ...
  "c8_lambda_search_policy_2026-06-05"
)
...
layouts <- data.frame(
  layout_id = c("right_boundary_mixed", "interior_positive",
                "left_boundary_positive"),
  description = c(
    "Includes zero and a positive grid expected to favor the right boundary.",
    "Positive grid centered around the previously favored lambda.",
    "Positive grid beginning at the previously favored lambda."
  ),
  lambda_grid = I(list(
...
for (ii in seq_len(nrow(cases))) {
  ...
  for (jj in seq_len(nrow(layouts))) {
    lambda.grid <- layouts$lambda_grid[[jj]]
    message(sprintf(
      "C8 evaluating %s %s %s",
      prepared$batch_id, prepared$chart_dim_rule, layouts$layout_id[[jj]]
    ))
    ...
    full.position <- if (full.best.raw$lambda.sync[[1L]] ==
                         min(lambda.grid[lambda.grid > 0])) {
      "left_boundary"
    } else if (full.best.raw$lambda.sync[[1L]] == max(lambda.grid)) {
      "right_boundary"
    } else if (full.best.raw$lambda.sync[[1L]] == 0) {
      "zero"
    } else {
      "interior"
    }
    ...
    rows[[rr]] <- data.frame(
      ...
      layout_id = layouts$layout_id[[jj]],
    ...
    full.table$layout_id <- layouts$layout_id[[jj]]
    ...
    if (nrow(search$stages)) {
      ...
      search$stages$layout_id <- layouts$layout_id[[jj]]
      ss <- ss + 1L
...
write.csv.safe(summary.table, file.path(table.dir, "ps_lps_c8_search_summary.csv"))
write.csv.safe(curve.table, file.path(table.dir, "ps_lps_c8_full_grid_curves.csv"))
write.csv.safe(stage.table, file.path(table.dir, "ps_lps_c8_search_stages.csv"))
...
curve.table$case_layout <- paste(curve.table$batch_id, curve.table$layout_id)
...
p.curve <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = "C8 full-grid CV curves and searched candidates",
  ...
  theme.report
ggplot2::ggsave(file.path(fig.dir, "ps_lps_c8_cv_curves.png"),
                p.curve, width = 13, height = 8, dpi = 180)

summary.table$case_layout <- paste(summary.table$batch_id,
...
p.count <- ggplot2::ggplot(
  ...
  ggplot2::coord_flip() +
  ggplot2::labs(
    title = "C8 candidate count reduction",
  ...
  theme.report
ggplot2::ggsave(file.path(fig.dir, "ps_lps_c8_candidate_counts.png"),
                p.count, width = 10, height = 6.5, dpi = 180)
...
p.regret <- ggplot2::ggplot(
  ...
  ggplot2::coord_flip() +
  ggplot2::labs(
    title = "C8 CV RMSE regret relative to full grid",
    subtitle = "Regret is search-selected CV RMSE minus full-grid selected CV RMSE.",
    x = NULL,
    y = "CV RMSE regret"
  ) +
  theme.report
ggplot2::ggsave(file.path(fig.dir, "ps_lps_c8_cv_regret.png"),
                p.regret, width = 10, height = 6.5, dpi = 180)
...
report <- htmltools::tagList(
  htmltools::tags$html(
    htmltools::tags$head(
      htmltools::tags$title("PS-LPS C8 Lambda Search Policy Prototype"),
      htmltools::tags$style(css)
    ),
    htmltools::tags$body(
      htmltools::tags$h1("PS-LPS C8 Lambda Search Policy Prototype"),
      htmltools::tags$p(
        class = "note",
        "C8 evaluates a guarded coarse-to-refine lambda.sync search policy ",
        "against full-grid references.  The deployable correctness guard is ",
        "CV RMSE regret: the search-selected CV RMSE minus the full-grid ",
        "selected CV RMSE.  A practical tie rule selects the smallest ",
        "lambda.sync whose CV RMSE is within 0.2% of the grid minimum."
      ),
      ...
      htmltools::tags$p(
        ...
        sprintf(
          paste0(
            "Across the C8 prototype cases, selected-lambda agreement was %s, ",
            "maximum CV RMSE regret was %s, and candidate reduction was at ",
            "least %s%%.  These are prototype results; they validate the ",
            "search-policy direction but do not yet establish a package default."
          ),
      ...
      htmltools::tags$div(
        class = "figure",
        htmltools::tags$img(src = file.path("figures",
                                            "ps_lps_c8_candidate_counts.png"))
      ),
      htmltools::tags$div(
        class = "figure",
        htmltools::tags$img(src = file.path("figures",
                                            "ps_lps_c8_cv_regret.png"))
      ),
      htmltools::tags$h2("Output Tables"),
      htmltools::tags$ul(
        htmltools::tags$li(htmltools::tags$a(
          href = file.path("tables", "ps_lps_c8_search_summary.csv"),
          "Search summary"
        )),
        htmltools::tags$li(htmltools::tags$a(
          href = file.path("tables", "ps_lps_c8_full_grid_curves.csv"),
          "Full-grid CV curves"
        )),
        htmltools::tags$li(htmltools::tags$a(
          href = file.path("tables", "ps_lps_c8_search_stages.csv"),
          "Search stages"
        ))
      )
    )
...
report.path <- file.path(out.dir, "ps_lps_c8_lambda_search_policy_report.html")
