#!/usr/bin/env python3
"""Failure-isolated launcher for broader P7X-style LPS/PS-LPS backend runs."""
...
def write_error_status(
    ...
    payload = {
        ...
        "error_class": error_class or f"worker_exit_{returncode}",
    }
...
def main() -> int:
    ...
    parser.add_argument("--task_timeout_sec", type=float, default=5400.0)
    ...
    worker = pathlib.Path(
        ...
        "run_lps_ps_lps_backend_broader_p7x_task.R"
    )
    merge = pathlib.Path(
        "/Users/pgajer/current_projects/geosmooth/scripts/"
        "merge_lps_ps_lps_backend_broader_p7x_run.R"
    )
    ...
    with launcher_log.open("a") as log:
        ...
        log.write(f"[{now()}] all workers complete; merging\n")
        log.flush()
        with (run_dir / "logs" / "merge_after_launch.log").open("w") as merge_log:
            subprocess.run(
                ["Rscript", str(merge), f"--run_dir={run_dir}"],
                stdout=merge_log,
                stderr=subprocess.STDOUT,
                check=False,
            )
