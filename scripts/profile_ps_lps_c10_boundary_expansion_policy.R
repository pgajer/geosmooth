...
out.dir <- file.path(
  ...
  "c10_boundary_expansion_policy_2026-06-05"
)
...
layout.id <- "wide_mixed_15_expand_2"
...
for (ii in seq_len(nrow(cases))) {
  ...
  message(sprintf(
    "C10 evaluating %s %s %s",
    prepared$batch_id, prepared$chart_dim_rule, layout.id
  ))

  full.evaluator <- make.evaluator(prepared)
  full.t <- time.block(full.evaluator$evaluate(reference.lambda.grid))
  full.table <- full.t$value
  full.selected <- .ps.lps.select.lambda.table(full.table, rel.tol = 0.002)
  ...
  full.position <- if (full.best.raw$lambda.sync[[1L]] == 0) {
    "zero"
             ...
             min(reference.lambda.grid[reference.lambda.grid > 0])) {
    "left_boundary"
  } else if (full.best.raw$lambda.sync[[1L]] == max(reference.lambda.grid)) {
    "right_boundary"
  } else {
    "interior"
  }
  ...
  search.t <- time.block(.ps.lps.search.lambda.sync(
    evaluate = function(lambda) search.evaluator$evaluate(lambda),
    lambda.grid = base.lambda.grid,
    control = list(
      rel.tol = 0.002,
      boundary.factor = 3,
  ...
  rows[[rr]] <- data.frame(
    ...
    layout_id = layout.id,
    initial_grid = paste(base.lambda.grid, collapse = ", "),
    reference_grid = paste(reference.lambda.grid, collapse = ", "),
    initial_candidate_count = length(base.lambda.grid),
    reference_candidate_count = length(reference.lambda.grid),
    search_candidate_count = nrow(search$evaluated),
    candidate_reduction = length(reference.lambda.grid) - nrow(search$evaluated),
    candidate_reduction_pct =
      100 * (length(reference.lambda.grid) - nrow(search$evaluated)) /
      length(reference.lambda.grid),
    boundary_expansion_count =
      sum(grepl("^boundary_expand_", search$telemetry$stage)),
  ...
  full.table$path <- "expanded_reference"
  ...
  if (nrow(search$telemetry)) {
    search$telemetry$batch_id <- prepared$batch_id
    search$telemetry$dataset_id <- prepared$dataset_id
    search$telemetry$chart_dim_rule <- prepared$chart_dim_rule
    search$telemetry$layout_id <- layout.id
    ss <- ss + 1L
    stages[[ss]] <- search$telemetry
  }
}
...
write.csv.safe(summary.table, file.path(table.dir, "ps_lps_c10_search_summary.csv"))
write.csv.safe(curve.table,
               file.path(table.dir, "ps_lps_c10_reference_grid_curves.csv"))
write.csv.safe(stage.table, file.path(table.dir, "ps_lps_c10_search_stages.csv"))
...
p.curve <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = "C10 expanded-reference CV curves and searched candidates",
    subtitle = "Green circle = expanded-reference selected lambda; orange cross = search-selected lambda.",
    x = "lambda.sync candidate",
    y = "CV RMSE",
    color = "Evaluated by search"
  ) +
  theme.report
ggplot2::ggsave(file.path(fig.dir, "ps_lps_c10_cv_curves.png"),
...
p.count <- ggplot2::ggplot(
  ...
  ggplot2::geom_col(
    ggplot2::aes(y = reference_candidate_count),
    fill = "gray82",
    width = 0.65
  ) +
  ggplot2::geom_point(
  ...
  ggplot2::coord_flip() +
  ggplot2::labs(
    title = "C10 candidate count reduction",
    subtitle = "Gray bars show expanded reference-grid candidate count; blue points show searched candidate count.",
    x = NULL,
    y = "Number of lambda.sync candidates"
  ) +
  theme.report
ggplot2::ggsave(file.path(fig.dir, "ps_lps_c10_candidate_counts.png"),
...
p.regret <- ggplot2::ggplot(
  ...
  ggplot2::coord_flip() +
  ggplot2::labs(
    title = "C10 CV RMSE regret relative to expanded reference",
    subtitle = "Regret is search-selected CV RMSE minus expanded-reference selected CV RMSE.",
    x = NULL,
    y = "CV RMSE regret"
  ) +
  theme.report
ggplot2::ggsave(file.path(fig.dir, "ps_lps_c10_cv_regret.png"),
...
display.summary <- display.summary[, c(
  ...
  "initial_candidate_count", "reference_candidate_count",
  "search_candidate_count", "candidate_reduction_pct",
  "boundary_expansion_count", "full_best_position",
...
report <- htmltools::tagList(
  htmltools::tags$html(
    htmltools::tags$head(
      htmltools::tags$title("PS-LPS C10 Boundary-Expansion Search Policy"),
      htmltools::tags$style(css)
    ),
    htmltools::tags$body(
      htmltools::tags$h1("PS-LPS C10 Boundary-Expansion Search Policy"),
      htmltools::tags$p(
        class = "note",
        "C10 evaluates the guarded coarse-to-refine lambda.sync search policy ",
        "with explicit boundary expansion across a frozen first-batch suite. ",
        "The expanded reference grid contains two extra candidates below the ",
        "initial positive lower bound and two extra candidates above the ",
        "initial upper bound.  The deployable ",
        "correctness guard is CV RMSE regret: the search-selected CV RMSE ",
        "minus the expanded-reference selected CV RMSE.  A practical tie rule selects ",
      ...
      htmltools::tags$p(
        ...
        sprintf(
          paste0(
            "Across the C10 validation cases, selected-lambda agreement was %s, ",
            "maximum CV RMSE regret was %s, and candidate reduction was at ",
            "least %s%% relative to the expanded reference grid.  These ",
            "validation results exercise the package helper, but they do not ",
            "yet establish a package default."
          ),
      ...
      htmltools::tags$h2("Expanded Reference Curves"),
      htmltools::tags$div(
        class = "figure",
        htmltools::tags$img(src = file.path("figures",
                                            "ps_lps_c10_cv_curves.png")),
        htmltools::tags$p(
          class = "caption",
          "Each panel shows the expanded reference CV curve. ",
          "Search-evaluated candidates are highlighted.  The green circle ",
          "marks the expanded-reference tie-rule selection and the orange ",
          "cross marks the search selection."
        )
      ),
      ...
      htmltools::tags$div(
        class = "figure",
        htmltools::tags$img(src = file.path("figures",
                                            "ps_lps_c10_candidate_counts.png"))
      ),
      htmltools::tags$div(
        class = "figure",
        htmltools::tags$img(src = file.path("figures",
                                            "ps_lps_c10_cv_regret.png"))
      ),
      htmltools::tags$h2("Output Tables"),
      htmltools::tags$ul(
        htmltools::tags$li(htmltools::tags$a(
          href = file.path("tables", "ps_lps_c10_search_summary.csv"),
          "Search summary"
        )),
        htmltools::tags$li(htmltools::tags$a(
          href = file.path("tables", "ps_lps_c10_reference_grid_curves.csv"),
          "Expanded-reference CV curves"
        )),
        htmltools::tags$li(htmltools::tags$a(
          href = file.path("tables", "ps_lps_c10_search_stages.csv"),
          "Search stages"
        ))
      )
    )
...
report.path <- file.path(out.dir, "ps_lps_c10_boundary_expansion_policy_report.html")
