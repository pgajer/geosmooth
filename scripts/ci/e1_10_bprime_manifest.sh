...
#
# Produces a self-contained, TRACKED bundle under reports/e1_10_bprime/ binding
...
#
# Usage: bash scripts/ci/e1_10_bprime_manifest.sh
# =============================================================================
set -uo pipefail
cd "$(git rev-parse --show-toplevel)" || exit 2

OUT="reports/e1_10_bprime"
...
FROZEN=(R/lps.R R/lps_cv_utils.R validation/e1_10_nested_grouped_cv.R \
        tests/testthat/test-lps-nested-grouped-cv.R)
BPRIME_SRC=(validation/e1_10_grouped_loco_bprime.R)
...
Rscript -e '
cc <- read.csv("reports/e1_10_bprime/e1_10_bprime_core_cases.csv")
write.csv(cc[, c("rho.nominal","replicate","seed.base","realized.icc")],
          "reports/e1_10_bprime/realized_rho_per_replicate.csv", row.names = FALSE)
' 2>&1 || true
